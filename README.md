...
<a href="https://www.bilibili.com/video/BV177411s7Lt" target="blank">视频链接</a>

## 总结

大概喽了眼文档，这个套视频介绍的东西十分之都不到，但是也算是一个比较好的入门视频吧，尤其像之前了解时听到最多的 **ipc** 的东西完全都没有讲，包括后期的**打包**完全没有涉及，给人的感觉更像是一个流产的系列(可能后续还会更新)。但也有认知上的收获，算是真切体会了一下什么叫 **node + chrom + 原生**了。

另外想记录一下学这一套遇到的非 electron 问题

1. 电脑装的 nvm 之前一直没有**单独设置 npm**，所有切一次版本就得重装全局的包，这次就索性换了，目前切 node 版本不影响全局的包。但**装不了 cnpm**，难受，最后考虑了到这不是重点，就在项目中添加了 npmrc。
2. 前面设置淘宝源之后第一次安装**低版本 electron**没问题，但删除**node_modules**之后还是装不上，应该是换了电脑之后 npm 版本不同的原因，最后装了`yarn`,成功安装。
3. 另外还在上周发现奇怪的问题，开谷歌上网助手后 github 无法访问，但另一台电脑上不受这个影响。包括设置过 host 的 github.io 也无法访问。最后确认是宽带原因，但还是不解。
4. 学这个的初衷是想把之前写的那个 node 爬虫添加一个非控制台操作界面，让用的人自己去操作。
5. 最后感慨下，node 还是重要呀
...
1. `globalShortcut`模块，主进程中的。有**全局快捷键**和**本地快捷键**之分，本节是按照全局快捷键来弄的。菜单项的快捷方式被称为本地快捷键，要求应用获得焦点。全局快捷键在应用未获得焦点时也能生效。另，官网有介绍在渲染进程中依赖`keyup`和第三方插件的方式。

## 剪切板的使用

1. 依赖`clipboard`模块，两种进程都可以使用，本节案例没什么太多可说的。看了眼文档，剪切板不仅可以写还可以读，该模块有多重方法，具体见文档。
2. 想起之前的一篇文章 http://www.ruanyifeng.com/blog/2021/01/clipboard-api.html 这里当时看它是基于富文本需求。
